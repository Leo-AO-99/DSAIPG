...
public final class FibonacciHeap<T> {
    ...
    public Entry<T> enqueue(T value, double priority) {
        ...
        Entry<T> overflow = null;
        if (mSize >= mMaxSize) {
            overflow = removeLowestPriorityEntry(priority);
            if (overflow == null) {
                return result;
            }
        }
    ...
    private Entry<T> removeLowestPriorityEntry(double newPriority) {
        if (isEmpty()) {
            return null;
        }
        
        if (newPriority > findLowestPriority()) {
            return null;
        }
        
        Entry<T> lowestEntry = findLowestPriorityEntry();
    ...
    private Entry<T> findLowestPriorityEntry() {
        if (isEmpty()) {
            return null;
        }
        
        Entry<T> lowestEntry = mMin;
        double lowestPriority = mMin.mPriority;
        
        Entry<T> current = mMin.mNext;
        while (current != mMin) {
            if (current.mPriority > lowestPriority) {
                lowestPriority = current.mPriority;
                lowestEntry = current;
            }
    ...
    private double findLowestPriority() {
        if (isEmpty()) {
